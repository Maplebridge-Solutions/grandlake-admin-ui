...
import { updateTicketEntry } from "@/lib/api/tickets";
...
const riderTypeLabel = (value: string) => RIDER_TYPES.find((r) => r.value === value)?.label ?? value;
const validityLabel = (days: number) => VALIDITY_OPTIONS.find((v) => v.days === days)?.label ?? `${days} days`;
...
export default function EditTicketTypeModal({ ticket, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (!ticket) return;
    setName(ticket.name);
    setCategory(categoryLabel(ticket.category));
    setRiderType(riderTypeLabel(ticket.riderType));
    setRidesCount(String(ticket.ridesCount));
    setPrice(ticket.price.toFixed(2));
    setValidityDays(validityLabel(ticket.validityDays));
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await updateTicketEntry(ticket._id, {
        name,
        description,
      ...
      <div className="bg-white w-full max-w-[95vw] sm:max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-4 sm:p-8 space-y-6 sm:space-y-8">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-content-primary tracking-tight">Edit Ticket Type</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={riderType} onValueChange={(v) => { setRiderType(v ?? ""); setErrors((p) => ({ ...p, riderType: undefined })); }}>
            ...
            <div className="space-y-1.5">
              ...
              <div className={`flex h-12 rounded-2xl border bg-surface-page transition-all ${errors.price ? "border-red-400" : "border-surface-subtle"}`}>
                <span className="flex items-center pl-4 pr-2 text-sm font-bold text-black shrink-0">$</span>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                ...
                />
                <span className="flex items-center pl-2 pr-4 text-sm font-bold text-black shrink-0">CAD</span>
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={validityDays} onValueChange={(v) => { setValidityDays(v ?? ""); setErrors((p) => ({ ...p, validityDays: undefined })); }}>
